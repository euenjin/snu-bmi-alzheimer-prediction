...
library(data.table)
uds_Data <- fread("data/uds_mock_Data.csv", data.table = FALSE)
str(uds_Data)
...
# 1. Create VISIT_DATE 
df_stage1_complete$VISIT_DATE <- with(df_stage1_complete, as.Date(paste(VISITYR, VISITMO, VISITDAY, sep = "-")))
...
# 1. Calculate visit order and delta_days
...
# 2. Filter subjects with 3 to 10 visits
df_filtered <- df_long %>%
  group_by(NACCID) %>%
  filter(n() >= 3 & n() <= 10) %>%
  ungroup()

# 3. Set max_visits fixed at 10
max_visits <- 10

# 4. Create all combinations of NACCID and visit_num from 1 to 10
full_seq <- expand.grid(
  NACCID = unique(df_filtered$NACCID),
  visit_num = 1:max_visits
)

# 5. Merge with filtered data and pad missing visits
df_padded <- full_seq %>%
  left_join(df_filtered, by = c("NACCID", "visit_num")) %>%
  group_by(NACCID) %>%
  arrange(visit_num, .by_group = TRUE) %>%
  tidyr::fill(everything(), .direction = "downup") %>%  # Fill missing values for all columns
  mutate(
    mask = ifelse(is.na(VISIT_DATE), 0, 1),            # Create mask: 1 if visit exists, 0 if padded
    VISIT_DATE = ifelse(mask == 0, as.Date(NA), VISIT_DATE),  # Set VISIT_DATE to NA for padded rows
    delta_days = ifelse(mask == 0, 0, delta_days)       # Set delta_days to 0 for padded rows
  ) %>%
  ungroup() %>%
