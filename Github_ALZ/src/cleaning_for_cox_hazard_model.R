...
# Load the dataset
cox_Data <- fread("data/cox_moc_data.csv", data.table = FALSE)
...
# --- Filter patients with more than two visits---
...
# 1. Extract First visit data from each patient
first_visit_features <- cox_Data_2plus %>%
  group_by(NACCID) %>%
  filter(VISIT_DATE == min(VISIT_DATE)) %>%
  ungroup() %>%
  select(-VISIT_DATE)  # VISIT_DATE is exluded as it is already captured in start_date

# 2. cox_final_with_features= time + features from first visit
